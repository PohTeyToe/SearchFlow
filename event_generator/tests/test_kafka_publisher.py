...
def mock_producer():
    ...
    with patch("src.publishers.Producer") as mock_producer_cls:
        instance = MagicMock()
        mock_producer_cls.return_value = instance
        yield instance
...
class TestCreatePublisherKafka:
    ...
    def test_output_all_creates_multi_publisher(self, mock_redis, mock_prod, tmp_path):
        mock_redis.return_value.ping.return_value = True
        pub = create_publisher("all", kafka_bootstrap_servers="localhost:9092", output_dir=str(tmp_path))
        assert isinstance(pub, MultiPublisher)
        assert len(pub.publishers) == 3

    def test_output_file_no_kafka(self, tmp_path):
        pub = create_publisher("file", output_dir=str(tmp_path))
